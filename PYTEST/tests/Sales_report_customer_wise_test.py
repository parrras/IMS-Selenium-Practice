...
def test_generate_sales_report_customer_wise(setup):
    ...
    try:
        ...
        # --- Step 2: Generate Sales Report Customer Wise ---
        sales_report_customer_page = SalesReportCustomerWisePage(driver)
        sales_report_customer_page.generate_sales_report_customer_wise()
        print("📊 Sales Report Customer Wise generated successfully.")

        # ✅ Step 3: Capture screenshot after successful report generation
        allure.attach(
            driver.get_screenshot_as_png(),
            name="Sales_Report_Customer_Wise_Success",
            attachment_type=allure.attachment_type.PNG
        )
        print("📸 Screenshot captured after successful Sales Report Customer Wise generation.")

    except Exception as e:
        # ❌ Step 4: Capture screenshot & error details on failure
        allure.attach(
            driver.get_screenshot_as_png(),
            name="Sales_Report_Customer_Wise_Error",
            attachment_type=allure.attachment_type.PNG
        )
        allure.attach(
            str(e),
        ...
        pytest.fail(f"❌ Sales Report Customer Wise test failed due to: {e}")
